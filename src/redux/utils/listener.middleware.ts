...
export const createListenerMiddleware = <S>() => {
  ...
  interface Listener {
    ...
    action: ListenerAction<any, any, string> | SmartListenerAction<any, string>;
  ...
  function addListenerOld<V extends string>(types: V | V[]) {
    return <T extends AppAction>(listenerAction: ListenerAction<S, T, V>) => {
      const action = {
        ...
        action: (listenerAction as any) as ListenerAction<any, any, string>,
      };
  ...
  function addSmartListener<V extends string>(type: V) {
    return <T extends AppAction>(listenerAction: SmartListenerAction<T, V>) => {
      const action = {
        ...
        async (actionContext) =>
          new Promise<any>((resolve) => {
            const registerPromise = {
              listenerType: ListenerType.ListenerAction,
              action: (({ action }) => resolve(action)) as ListenerAction<
                any,
                any,
                string
              >,
  ...
  async function mergeOnceListener<V extends AppActionContextArgs>(
    ...args: [V | V[], MergeOnceListenerCallback] | [MergeOnceListenerPath[]]
  ) {
    const data =
      args.length === 1
        ? await Promise.race(
            args[0].map(({ actionsContexts, callback }) =>
              waitForAllActionsToBeDispatched(actionsContexts, callback)
            )
          )
        ...
        case ListenerType.ListenerAction:
          const { getState, dispatch } = config.generateGraph
            ? (overrideStoreForGraph(
                store,
                listener.merge
                  ? ({ type: "MergeOnceListenerAction" } as AppAction)
                  : action
              ) as MiddlewareAPI)
            : store;
          ...
          if (listener.destructuringArgs) {
            await (listener.action as ListenerActionFromContext<S, any>)({
              getState,
              action,
        ...
        case ListenerType.SmartListenerAction:
          return (listener.action as SmartListenerAction<any, any>)(
            config.generateGraph
              ? (overrideStoreForGraph(store, action) as MiddlewareAPI)
              : store,
            action
          );
      }
